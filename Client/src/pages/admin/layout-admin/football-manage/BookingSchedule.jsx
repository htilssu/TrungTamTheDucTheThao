...
const BookingSchedule = () => {
    ...
    const handleCancelBooking = () => {
        setBookings(bookings.filter(b => !(b.date === modalBooking.date && b.time === modalBooking.time && b.field === modalBooking.field)));
        setModalOpen(false);
        setModalBooking(null);
    };
                        ...
                        {times.map((time, index) => (
                            <tr key={`${selectedField}-${s}-${index}`}>
                                ...
                                {dates.map(({ date }) => {
                                    ...
                                    const booking = bookings.find(booking => booking.date === date && booking.time === time && booking.field === `${selectedField}-${s}`);
                                    return (
                                        <td
                                            key={`${date}-${time}`}
                                            className={`border border-gray-300 p-2 ${booking ? 'bg-green-200 cursor-pointer' : 'cursor-pointer hover:bg-blue-100'}`}
                                            onClick={() => booking ? openEditModal(booking) : openBookingModal(date, time, `${selectedField}-${s}`)} // Mở modal đặt lịch hoặc chỉnh sửa
                                        >
                                            {booking ? (
                                                <div>
